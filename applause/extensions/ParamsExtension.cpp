...
std::string ParamsExtension::defaultValueToText(float value,
    ...
    if (info.stepped) {
        ...
        int integer_digits = (abs_value >= 1.0f)
                                 ? static_cast<int>(std::log10(abs_value)) + 1
                                 : 1;
        ...
        if (used_chars >= max_chars) {
            ...
            if (available_for_decimal >= 2) {
                int decimals_to_show =
                    std::min(max_decimals, available_for_decimal - 1);
                stream << std::fixed << std::setprecision(decimals_to_show)
                       << value;
            } else {
                stream << std::fixed << std::setprecision(0) << value;
            }
...
std::optional<float> ParamsExtension::defaultTextToValue(
    const std::string& text, const ParamInfo& info) {
    if (text.empty()) {
        return std::nullopt;
    }
    ...
    while (*str && !std::isdigit(*str) && *str != '-' && *str != '+' &&
           *str != '.') {
        ++str;
    }
...
void ParamInfo::setValueNotifyingHost(float value) const noexcept {
    ...
    if (registry_->message_queue_) {
        registry_->message_queue_->toAudio().enqueue(
            {ParamMessageQueue::PARAM_VALUE, clapId, value});
    }

    handle_->value_->store(std::clamp(value, minValue, maxValue),
...
void ParamInfo::beginGesture() const noexcept {
    ...
    if (registry_->message_queue_) {
        registry_->message_queue_->toAudio().enqueue(
            {ParamMessageQueue::BEGIN_GESTURE, clapId, 0.0f});
    }
...
void ParamInfo::endGesture() const noexcept {
    ...
    if (registry_->message_queue_) {
        registry_->message_queue_->toAudio().enqueue(
            {ParamMessageQueue::END_GESTURE, clapId, 0.0f});
    }
...
std::optional<float> ParamInfo::textToValue(
    const std::string& text) const noexcept {
    return text_to_value_(text, *this);
}

uint32_t ParamsExtension::clap_params_count(
...
bool ParamsExtension::clap_params_get_info(
...
bool ParamsExtension::clap_params_get_value(const clap_plugin_t* plugin,
    ...
    *out_value = static_cast<double>(
        ext->values_[index].load(std::memory_order_relaxed));

    return true;
}

bool ParamsExtension::clap_params_value_to_text(
...
void ParamsExtension::clap_params_flush(
...
void ParamsExtension::registerParam(const ParamConfig& config) {
    ...
    ASSERT(config.default_value >= config.min_value &&
               config.default_value <= config.max_value,
    ...
    infos_[index].value_to_text_ =
        config.value_to_text ? config.value_to_text : defaultValueToText;
    infos_[index].text_to_value_ =
        config.text_to_value ? config.text_to_value : defaultTextToValue;
...
void ParamsExtension::processEvents(const clap_input_events_t* in,

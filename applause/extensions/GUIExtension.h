 ...
 * with CLAP hosts.
 *
 * The extension doesn't implement a GUI; rather, it is a C++ glue
 * layer that lets generic UI editors communicate with the host application and
 * respond to parameter changes.
 *
 * Therefore, there is an extra layer of indirection. You can use any GUI system you would like,
 * as long as your editor class is compliant with the IEditor interface.
 *
 * Of course, we recommend the built-in GPU-accelerated ApplauseEditor, which makes
...
public:
     ...
     */
    explicit GUIExtension(EditorFactory factory, uint32_t defaultWidth = 800, uint32_t defaultHeight = 600,
...
private:
    ...
    static bool clap_gui_is_api_supported(const clap_plugin_t* plugin, const char* api, bool is_floating) noexcept;

    static bool clap_gui_get_preferred_api(const clap_plugin_t* plugin, const char** api, bool* is_floating) noexcept;

    static bool clap_gui_create(const clap_plugin_t* plugin, const char* api, bool is_floating) noexcept;

    static void clap_gui_destroy(const clap_plugin_t* plugin) noexcept;

    static bool clap_gui_set_scale(const clap_plugin_t* plugin, double scale) noexcept;

    static bool clap_gui_get_size(const clap_plugin_t* plugin, uint32_t* width, uint32_t* height) noexcept;

    static bool clap_gui_can_resize(const clap_plugin_t* plugin) noexcept;

    static bool clap_gui_get_resize_hints(const clap_plugin_t* plugin, clap_gui_resize_hints_t* hints) noexcept;

    static bool clap_gui_adjust_size(const clap_plugin_t* plugin, uint32_t* width, uint32_t* height) noexcept;

    static bool clap_gui_set_size(const clap_plugin_t* plugin, uint32_t width, uint32_t height) noexcept;

    static bool clap_gui_set_parent(const clap_plugin_t* plugin, const clap_window_t* window) noexcept;

    static bool clap_gui_set_transient(const clap_plugin_t* plugin, const clap_window_t* window) noexcept;

    static void clap_gui_suggest_title(const clap_plugin_t* plugin, const char* title) noexcept;
    ...
    static constexpr clap_plugin_gui_t clap_struct_ = {.is_api_supported = clap_gui_is_api_supported,

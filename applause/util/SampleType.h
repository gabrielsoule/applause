...
namespace applause
{
    ...
    /**
     * A concept for a sample type. This can either be a scalar type, e.g. a float or a double, or a SIMD batch type,
     * e.g. a xsimd::batch<float/double>.
    */
    template <typename T>
    ...
    public:
        ...
        template <typename V>
            ...
            (std::convertible_to<V, S> || std::convertible_to<V, scalar_type>)
        ...
        friend constexpr StereoSample operator+(StereoSample lhs, const StereoSample& rhs) noexcept
        {
            lhs += rhs;
            return lhs;
        }
        ...
        template <typename V>
            requires (!std::same_as<std::remove_cvref_t<V>, StereoSample>) &&
            (std::convertible_to<V, S> || std::convertible_to<V, scalar_type>)
        friend constexpr StereoSample operator+(StereoSample lhs, V&& value) noexcept
        {
        ...
        template <typename V>
            requires (!std::same_as<std::remove_cvref_t<V>, StereoSample>) &&
            (std::convertible_to<V, S> || std::convertible_to<V, scalar_type>)
        friend constexpr StereoSample operator+(V&& value, StereoSample rhs) noexcept
        {
        ...
        template <typename V>
            requires (!std::same_as<std::remove_cvref_t<V>, StereoSample>) &&
            (std::convertible_to<V, S> || std::convertible_to<V, scalar_type>)
        friend constexpr StereoSample operator-(V&& value, const StereoSample& rhs) noexcept
        {
        ...
        friend constexpr StereoSample operator-(const StereoSample& value) noexcept
        {
    ...
    private:
        ...
        static constexpr S toSample(V&& value) noexcept
        {
            ...
            {
                static_assert(std::same_as<std::remove_cvref_t<V>, void>,
    ...
    {
        if constexpr (SimdBatch<S>) return S(v);
        else return v;
    }
    ...
    {
        if constexpr (SimdBatch<S>) return S{}.load_unaligned(p);
        else return *p;
    }
    ...
    {
        if constexpr (SimdBatch<S>) v.store_unaligned(p);
        else *p = v;
    }
    ...
    {
        if constexpr (SimdBatch<S>) return S{}.load_aligned(p);
        else return *p;
    }
    ...
    {
        if constexpr (SimdBatch<S>) v.store_aligned(p);
        else *p = v;
    }
    ...
    {
        if constexpr (SimdBatch<S>) return xsimd::fma(a, b, c);
    ...
    {
        if constexpr (SimdBatch<S>) return xsimd::min(a, b);
    ...
    {
        if constexpr (SimdBatch<S>) return xsimd::max(a, b);
    ...
    {
        if constexpr (SimdBatch<S>) return xsimd::abs(a);
    ...
    {
        if constexpr (SimdBatch<S>) return xsimd::sqrt(a);
        else return std::sqrt(a);
    }

    template <typename T>
    inline T select(const mask_t<T>& mask, const T& true_val, const T& false_val)
    {
        if constexpr (SimdBatch<T>) return xsimd::select(mask, true_val, false_val);
        else return mask ? true_val : false_val;
    }
} // namespace applause

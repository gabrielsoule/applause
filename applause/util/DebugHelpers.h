...
inline std::string timestamp() {
    ...
    return FMT_NAMESPACE::format("{:02d}:{:02d}:{:02d}.{:03d}", tm->tm_hour,
...
void log(Level level, std::string_view file, int line, std::string_view func,
    ...
    std::cout << FMT_NAMESPACE::format("[{}] {} {}:{} ({}) {}", timestamp(),
                                       level_name(level), filename, line, func,
                                       msg)
              << std::endl;
}
...
#define LOG_TRACE(...) \
    debug::log(debug::Level::TRACE, __FILE__, __LINE__, __func__, __VA_ARGS__)
#define LOG_DBG(...) \
    debug::log(debug::Level::DBG, __FILE__, __LINE__, __func__, __VA_ARGS__)
#define LOG_INFO(...) \
    debug::log(debug::Level::INFO, __FILE__, __LINE__, __func__, __VA_ARGS__)
#define LOG_WARN(...) \
    debug::log(debug::Level::WARN, __FILE__, __LINE__, __func__, __VA_ARGS__)
#define LOG_ERR(...) \
    debug::log(debug::Level::ERR, __FILE__, __LINE__, __func__, __VA_ARGS__)

 ...
 *                                                                                 *
 *   v2.1 - public domain * no warranty is offered or implied; use this code at
 *your own risk             *
 *                                                                                 *
 * AUTHORS *
 *                                                                                 *
...
/***********************************************************************************
 * -------------------------------------------------------------------------------
 * * Define this if you want to disable `set_timeout` and `set_interval`
 * features.   *
 * -------------------------------------------------------------------------------
 */
 ...
 * * For Visual Studio < 2015 you can define it to `__declspec(thread)` for
 * example. *
 * -------------------------------------------------------------------------------
 */
...
struct invocation_slot_error final : error {
    const char* what() const noexcept override {
        return "rocket: One of the slots has raised an exception during the "
               "signal invocation.";
    }
};
#endif
...
struct intrusive_ptr final {
    ...
    constexpr intrusive_ptr() noexcept : ptr{nullptr} {}

    constexpr intrusive_ptr(std::nullptr_t) noexcept : ptr{nullptr} {}

    explicit intrusive_ptr(pointer p) noexcept : ptr{p} {
        if (ptr) {
            ptr->addref();
        }
    }

    intrusive_ptr(intrusive_ptr const& p) noexcept : ptr{p.ptr} {
        if (ptr) {
            ptr->addref();
        }
    }

    intrusive_ptr(intrusive_ptr&& p) noexcept : ptr{p.ptr} { p.ptr = nullptr; }

    template <class U>
    explicit intrusive_ptr(intrusive_ptr<U> const& p) noexcept : ptr{p.ptr} {
        if (ptr) {
            ptr->addref();
        }
    }
    ...
    template <class U>
    explicit intrusive_ptr(intrusive_ptr<U>&& p) noexcept : ptr{p.ptr} {
        p.ptr = nullptr;
    }
...
ROCKET_NODISCARD inline bool operator==(intrusive_ptr<T> const& a,
...
template <class T, class U>
ROCKET_NODISCARD inline bool operator==(T* a,
                                        intrusive_ptr<U> const& b) noexcept {
    return a == b.get();
}
...
template <class T>
ROCKET_NODISCARD inline bool operator==(std::nullptr_t,
...
template <class T, class U>
ROCKET_NODISCARD inline intrusive_ptr<T> static_pointer_cast(
    intrusive_ptr<U> const& p) noexcept {
    return intrusive_ptr<T>{static_cast<T*>(p.get())};
}

template <class T, class U>
ROCKET_NODISCARD inline intrusive_ptr<T> const_pointer_cast(
    intrusive_ptr<U> const& p) noexcept {
    return intrusive_ptr<T>{const_cast<T*>(p.get())};
}

template <class T, class U>
ROCKET_NODISCARD inline intrusive_ptr<T> dynamic_pointer_cast(
    intrusive_ptr<U> const& p) noexcept {
    return intrusive_ptr<T>{dynamic_cast<T*>(p.get())};
}

template <class T, class U>
ROCKET_NODISCARD inline intrusive_ptr<T> reinterpret_pointer_cast(
    intrusive_ptr<U> const& p) noexcept {
...
class stable_list final {
    struct link_element final : ref_counted<link_element> {
        ...
        template <class... Args>
        void construct(Args&&... args) noexcept(noexcept(T{
   ...
   public:
    ...
    struct iterator_base final {
        ...
        iterator_base(iterator_base const& i) noexcept : element{i.element} {}
        ...
        template <class V>
        ROCKET_NODISCARD bool operator==(
            iterator_base<V> const& i) const noexcept {
    ...
    iterator insert(iterator const& pos, size_type count,
...
struct shared_lock_state<thread_safe_policy> final {
    ...
    shared_lock_state() : lock_primitive{new shared_lock} {}
...
struct connection_base<thread_unsafe_policy>
    ...
    void disconnect() noexcept {
        if (prev != nullptr) {
            ...
            // To mark a connection as disconnected, just set its prev-link to
            // null but leave the next link alive so we can still traverse
            // through the connections if the slot gets disconnected during
            // signal emit.
            prev = nullptr;
        }
    }
...
#ifndef ROCKET_NO_TIMERS
struct timed_connection final
    : functional_connection<thread_unsafe_policy, void()> {
...
struct connection_scope final {
    connection_scope(void* base, bool is_thread_safe,
                     thread_local_data* th) noexcept
        : th{th},
...
struct abort_scope final {
    abort_scope(thread_local_data* th) noexcept
        : th{th}, prev{th->emission_aborted} {
...
struct shared_mem_fn final {
    explicit shared_mem_fn(std::shared_ptr<Instance> c,
                           R (Class::*method)(Args...))
...
#ifndef ROCKET_NO_SMARTPOINTER_EXTENSIONS
template <class Instance, class Class, class R, class... Args>
inline auto bind_weak_ptr(std::weak_ptr<Instance> c,
                          R (Class::*method)(Args...)) {
    return detail::weak_mem_fn<Instance, Class, R, Args...>{std::move(c),
                                                            method};
}

template <class Instance, class Class, class R, class... Args>
inline auto bind_weak_ptr(std::shared_ptr<Instance> c,
                          R (Class::*method)(Args...)) {
    return detail::weak_mem_fn<Instance, Class, R, Args...>{std::move(c),
                                                            method};
}
...
template <class Instance, class Class, class R, class... Args>
inline auto bind_shared_ptr(std::shared_ptr<Instance> c,
                            R (Class::*method)(Args...)) {
    return detail::shared_mem_fn<Instance, Class, R, Args...>{std::move(c),
                                                              method};
}
#endif

struct connection {
    connection() noexcept : base{nullptr}, is_thread_safe{false} {}

    ~connection() noexcept { release(); }

    connection(connection&& rhs) noexcept
        : base{rhs.base}, is_thread_safe{rhs.is_thread_safe} {
        rhs.base = nullptr;
    ...
    explicit connection(void* base, bool is_thread_safe) noexcept
        : base{base}, is_thread_safe{is_thread_safe} {
        addref();
    }
    ...
    ROCKET_NODISCARD bool is_connected() const noexcept {
        if (base != nullptr) {
            if (is_thread_safe) {
                auto safe{
                    static_cast<detail::connection_base<thread_safe_policy>*>(
                        base)};
                return std::launder(safe)->is_connected();
            } else {
                auto unsafe{
                    static_cast<detail::connection_base<thread_unsafe_policy>*>(
                        base)};
...
struct scoped_connection final : connection {
    ...
    scoped_connection(connection const& rhs) noexcept : connection{rhs} {}

    scoped_connection(connection&& rhs) noexcept : connection{std::move(rhs)} {}
...
struct scoped_connection_container final {
    ...
    scoped_connection_container& operator+=(
        std::initializer_list<connection> list) {
        for (auto const& connection : list) {
            append(connection);
        }
        return *this;
...
struct scoped_connection_blocker final {
    scoped_connection_blocker(connection c) noexcept : conn{std::move(c)} {
        conn.block();
    }
...
struct timer_queue final {
    ...
    connection set_interval(
        slot_type slot, std::chrono::duration<Rep, Period> const& interval) {
        assert(slot != nullptr);

        auto expires_at = std::chrono::steady_clock::now() + interval;
        auto interval_microsecs =
            std::chrono::duration_cast<std::chrono::microseconds>(interval);
        connection_base* base =
            make_link(tail, std::move(slot), std::move(expires_at),
                      std::move(interval_microsecs));
    ...
    template <class Instance, class Class, class R, class Rep = unsigned long,
              class Period = std::milli>
    connection set_interval(
        Instance& object, R (Class::*method)(),
        std::chrono::duration<Rep, Period> const& interval) {
        connection c{set_interval<Rep, Period>(
            [&object, method] { (object.*method)(); }, interval)};
    ...
    template <auto Method, class Instance, class Rep = unsigned long,
              class Period = std::milli>
    connection set_interval(
        Instance& object, std::chrono::duration<Rep, Period> const& interval) {
        connection c{set_interval<Rep, Period>(
            [&object] { (object.*Method)(); }, interval)};
        if constexpr (std::is_convertible_v<Instance*, trackable*>) {
            static_cast<trackable&>(object).add_tracked_connection(c);
        }
        return c;
    ...
    template <class Rep = unsigned long, class Period = std::milli>
    connection set_timeout(slot_type slot,
    ...
    template <class Instance, class Class, class R, class Rep = unsigned long,
              class Period = std::milli>
    connection set_timeout(Instance& object, R (Class::*method)(),
                           std::chrono::duration<Rep, Period> const& timeout) {
        connection c{set_timeout<Rep, Period>(
            [&object, method] { (object.*method)(); }, timeout)};
        if constexpr (std::is_convertible_v<Instance*, trackable*>) {
            static_cast<trackable&>(object).add_tracked_connection(c);
        }
        return c;
    ...
    template <auto Method, class Instance, class Rep = unsigned long,
              class Period = std::milli>
    connection set_timeout(Instance& object,
                           std::chrono::duration<Rep, Period> const& timeout) {
        connection c{set_timeout<Rep, Period>([&object] { (object.*Method)(); },
                                              timeout)};
        if constexpr (std::is_convertible_v<Instance*, trackable*>) {
            static_cast<trackable&>(object).add_tracked_connection(c);
        }
                ...
                if (current->prev != nullptr
                    ...
                    ROCKET_LIKELY {
                        ...
                        timed_connection* conn =
                            std::launder(static_cast<timed_connection*>(
                        ...
                        if (conn->expires_at <= now) {
                            ...
                            if (execute_until !=
                                ...
                                ROCKET_UNLIKELY {
                                    // Check if we already spent the maximum
                                    // allowed time executing callbacks
                                    if (execute_until <=
                                        std::chrono::steady_clock::now()) {
   ...
   private:
    ...
    void copy(timer_queue const& q) {
        ...
        while (current != end) {
            timed_connection* conn = std::launder(
                static_cast<timed_connection*>(static_cast<void*>(current)));
    ...
    timed_connection* make_link(
        connection_base* l, slot_type slot,
        std::chrono::time_point<std::chrono::steady_clock> expires_at,
        std::chrono::microseconds interval) {
    ...
    bool dispatch(
        ...
        while (itr != end) {
            ...
            if (execute_until !=
                ...
                ROCKET_UNLIKELY {
                    // check if we already spent the maximum allowed time
                    // executing callbacks
        ...
        if (itr != end) ROCKET_UNLIKELY {
                ...
                std::scoped_lock<std::mutex> guard{mutex};
                std::deque<std::packaged_task<void()>>& original_queue =
                    queue[tid];
    ...
    typedef typename std::conditional<
        ...
        typename std::conditional<
            ...
            typename std::conditional<
                ...
                typename std::remove_cv<U>::type, T>::type>::type>::type type;
};
...
inline void dispatch_queued_calls(
    std::chrono::duration<Rep, Period> const& max_time_to_execute) {
    std::chrono::time_point<std::chrono::steady_clock> execute_until{};
    if (max_time_to_execute.count() > 0) ROCKET_UNLIKELY {
            execute_until =
                std::chrono::steady_clock::now() + max_time_to_execute;
        }
#ifndef ROCKET_NO_TIMERS
...
inline connection set_interval(
    ...
    std::chrono::duration<Rep, Period> const& interval) {
    return detail::get_timer_queue()->template set_interval<Rep, Period>(
        std::move(slot), interval);
}

template <auto Method, class Rep = unsigned long, class Period = std::milli>
inline connection set_interval(
    std::chrono::duration<Rep, Period> const& interval) {
    return detail::get_timer_queue()
        ->template set_interval<Method, Rep, Period>(interval);
}
...
inline connection set_interval(
    ...
    std::chrono::duration<Rep, Period> const& interval) {
    return detail::get_timer_queue()
        ->template set_interval<Instance, Class, R, Rep, Period>(object, method,
                                                                 interval);
}
...
inline connection set_interval(
    Instance& object, std::chrono::duration<Rep, Period> const& interval) {
    return detail::get_timer_queue()
        ->template set_interval<Method, Instance, Rep, Period>(object,
                                                               interval);
}
...
inline connection set_timeout(
    std::function<void()> slot,
    std::chrono::duration<Rep, Period> const& timeout) {
    return detail::get_timer_queue()->template set_timeout<Rep, Period>(
        std::move(slot), timeout);
}

template <auto Method, class Rep = unsigned long, class Period = std::milli>
inline connection set_timeout(
    std::chrono::duration<Rep, Period> const& timeout) {
    return detail::get_timer_queue()->template set_timeout<Method, Rep, Period>(
        timeout);
}
...
inline connection set_timeout(
    ...
    std::chrono::duration<Rep, Period> const& timeout) {
    return detail::get_timer_queue()
        ->template set_timeout<Instance, Class, R, Rep, Period>(object, method,
                                                                timeout);
}
...
template <auto Method, class Instance, class Rep = unsigned long,
          class Period = std::milli>
inline connection set_timeout(
    Instance& object, std::chrono::duration<Rep, Period> const& timeout) {
    return detail::get_timer_queue()
        ->template set_timeout<Method, Instance, Rep, Period>(object, timeout);
}
...
inline connection set_interval(std::function<void()> slot,
...
inline connection set_interval(unsigned long interval_ms) {
    return detail::get_timer_queue()->template set_interval<Method>(
...
inline connection set_timeout(std::function<void()> slot,
...
inline connection set_timeout(unsigned long timeout_ms) {
    return detail::get_timer_queue()->template set_timeout<Method>(
...
template <class Signature,
          class Collector =
              default_collector<detail::get_return_type<Signature>>,
...
struct signal<R(Args...), Collector, ThreadingPolicy> final {
    ...
    signal& operator=(signal const& rhs) {
        if (this != &rhs) {
            std::scoped_lock<shared_lock_state, shared_lock_state> guard{
                lock_state, rhs.lock_state};
            clear_without_lock();
            copy(rhs);
        }
        return *this;
    ...
    connection connect(slot_type slot,
                       connection_flags flags = direct_connection) {
        assert(slot != nullptr);
        ...
        std::scoped_lock<shared_lock_state> guard{lock_state};
        connection_base* base =
            make_link(first ? head->next : tail, std::move(slot)
#ifndef ROCKET_NO_QUEUED_CONNECTIONS
                                                     ,
                      tid
#endif
    ...
    connection connect(R1 (*method)(Args1...),
                       connection_flags flags = direct_connection) {
        return connect(
            [method](Args const&... args) {
    ...
    connection connect(connection_flags flags = direct_connection) {
        return connect(
            [](Args const&... args) { return R((*Method)(args...)); }, flags);
    }

    template <class Instance, class Class, class R1, class... Args1>
    connection connect(Instance& object, R1 (Class::*method)(Args1...),
                       connection_flags flags = direct_connection) {
        connection c{connect(
            [&object, method](Args const&... args) {
    ...
    template <auto Method, class Instance>
    connection connect(Instance& object,
                       connection_flags flags = direct_connection) {
        connection c{connect(
            [&object](Args const&... args) {
                ...
                if (current->prev != nullptr
                    ...
                    ROCKET_LIKELY {
                        detail::connection_scope cscope{
                            current,
                            std::is_same_v<threading_policy,
                                           thread_safe_policy>,
                            th};
                        ...
                        if constexpr (std::is_same_v<threading_policy,
                            ...
                            if (current->is_queued()) ROCKET_UNLIKELY {
                                    if constexpr (std::is_void_v<R>) {
                                        std::packaged_task<void()> task(
                                            [current, args = detail::make_tuple(
                                                          args...)] {
                                        ...
                                        detail::get_call_queue()->put(
                                            current->get_tid(),
                                            std::move(task));
                                    } else {
                                        // If we are calling a queued slot, and
                                        // our signal requires a return value we
                                        // actually have to block the thread
                                        // until the slot was dispatched
                                        std::packaged_task<void()> task(
                                            [current, &collector,
   ...
   private:
    ...
    using functional_connection =
        detail::functional_connection<threading_policy, signature_type>;
...
template <class Instance, class Class, class R, class... Args>
ROCKET_NODISCARD inline std::function<R(Args...)> slot(
    Instance& object, R (Class::*method)(Args...)) {

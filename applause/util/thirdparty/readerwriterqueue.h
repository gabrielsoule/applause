...
#ifndef MOODYCAMEL_MAYBE_ALIGN_TO_CACHELINE
#define MOODYCAMEL_MAYBE_ALIGN_TO_CACHELINE APPLAUSE_ALIGN(MOODYCAMEL_CACHE_LINE_SIZE)
#endif
...
public:
	...
	APPLAUSE_NO_TSAN explicit ReaderWriterQueue(size_t size = 15)
#ifndef NDEBUG
		: enqueuing(false)
		,dequeuing(false)
#endif
	...
	APPLAUSE_NO_TSAN ReaderWriterQueue(ReaderWriterQueue&& other)
	...
	// Note: The queue should not be accessed concurrently while it's
	// being moved. It's up to the user to synchronize this.
	ReaderWriterQueue& operator=(ReaderWriterQueue&& other) APPLAUSE_NO_TSAN
	{
	...
	APPLAUSE_NO_TSAN ~ReaderWriterQueue()
	{
	...
	APPLAUSE_FORCEINLINE bool try_enqueue(T const& element) APPLAUSE_NO_TSAN
	{
	...
	// Returns true if the element was enqueued, false otherwise.
	// Does not allocate memory.
	APPLAUSE_FORCEINLINE bool try_enqueue(T&& element) APPLAUSE_NO_TSAN
	{
	...
	APPLAUSE_FORCEINLINE bool try_emplace(Args&&... args) APPLAUSE_NO_TSAN
	{
	...
	APPLAUSE_FORCEINLINE bool enqueue(T const& element) APPLAUSE_NO_TSAN
	{
	...
	// Allocates an additional block of memory if needed.
	// Only fails (returns false) if memory allocation fails.
	APPLAUSE_FORCEINLINE bool enqueue(T&& element) APPLAUSE_NO_TSAN
	{
	...
	template<typename... Args>
	APPLAUSE_FORCEINLINE bool emplace(Args&&... args) APPLAUSE_NO_TSAN
	{
	...
	template<typename U>
	bool try_dequeue(U& result) APPLAUSE_NO_TSAN
	{
#ifndef NDEBUG
	...
	inline size_t size_approx() const APPLAUSE_NO_TSAN
	{
		size_t result = 0;
		Block* frontBlock_ = frontBlock.load();
		Block* block = frontBlock_;
...
private:
	...
	bool inner_enqueue(Args&&... args) APPLAUSE_NO_TSAN
#else
	template<AllocationMode canAlloc, typename U>
	bool inner_enqueue(U&& element) APPLAUSE_NO_TSAN
#endif
	{
#ifndef NDEBUG
	...
	APPLAUSE_FORCEINLINE static size_t ceilToPow2(size_t x)
	{
	...
	template<typename U>
	static APPLAUSE_FORCEINLINE char* align_for(char* ptr) APPLAUSE_NO_TSAN
	{
...
private:
	...
	struct Block
	{
		...
		APPLAUSE_NO_TSAN Block(size_t const& _size, char* _rawThis, char* _data)
	...
	static Block* make_block(size_t capacity) APPLAUSE_NO_TSAN
	{
...
public:
	...
	BlockingReaderWriterQueue(BlockingReaderWriterQueue&& other) APPLAUSE_NO_TSAN
		: inner(std::move(other.inner)), sema(std::move(other.sema))
	{ }

	BlockingReaderWriterQueue& operator=(BlockingReaderWriterQueue&& other) APPLAUSE_NO_TSAN
	{
	...
	template<typename U>
	void wait_dequeue(U& result) APPLAUSE_NO_TSAN
	{
		while (!sema->wait());
		bool success = inner.try_dequeue(result);
		APPLAUSE_UNUSED(result);
		assert(success);
		APPLAUSE_UNUSED(success);
	}
	...
	template<typename U>
	bool wait_dequeue_timed(U& result, std::int64_t timeout_usecs) APPLAUSE_NO_TSAN
	{
	...
	inline bool wait_dequeue_timed(U& result, std::chrono::duration<Rep, Period> const& timeout) APPLAUSE_NO_TSAN
	{
	...
	// returned instead.
	// Must be called only from the consumer thread.
	APPLAUSE_FORCEINLINE T* peek() const APPLAUSE_NO_TSAN
	{
		return inner.peek();
	}
	...
	// Returns true on success, or false if the queue appeared empty at the time
	// `pop` was called.
	APPLAUSE_FORCEINLINE bool pop() APPLAUSE_NO_TSAN
	{
		if (sema->tryWait()) {
	...
	// Returns the approximate number of items currently in the queue.
	// Safe to call from both the producer and consumer threads.
	APPLAUSE_FORCEINLINE size_t size_approx() const APPLAUSE_NO_TSAN
	{
	...
	//       the case where the producer was writing to the same block the consumer was
	//       reading from the whole time.
	APPLAUSE_FORCEINLINE size_t max_capacity() const {

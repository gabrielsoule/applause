    ...
    struct MemoryArena
    {
       ...
       */
        void* allocateBytes(size_t num_bytes, size_t alignment = 1)
        {
            ...
            const auto bytes_increment = static_cast<size_t>(std::distance(
                raw_data_.data() + bytes_used_, pointer + num_bytes));
       ...
       */
        ...
        [[nodiscard]] BufferView<Sample, Channels> allocateAudioBuffer(std::size_t frame_count,
                                                                       std::size_t alignment = defaultByteAlignment)
        {
            ...
            const std::size_t effective_alignment = std::max(alignment, sample_alignment);

            constexpr std::size_t width = sampleWidth<Sample>();

            ASSERT(frame_count <= std::numeric_limits<std::size_t>::max() / width, "Frame count overflow!");
            const std::size_t scalars_per_channel = frame_count * width;
            ASSERT(scalars_per_channel <= std::numeric_limits<std::size_t>::max() / Channels, "Sample count overflow!");
            ...
            ASSERT(storage != nullptr, "Storage allocation failed! This should never happen.");

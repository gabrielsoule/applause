...
ExampleMIDIPrinterPlugin::ExampleMIDIPrinterPlugin(const clap_plugin_descriptor_t* descriptor,
                                                   const clap_host_t* host) :
    PluginBase(descriptor, host), note_ports_(), event_count_(0) {
...
bool ExampleMIDIPrinterPlugin::init() noexcept { return true; }

void ExampleMIDIPrinterPlugin::destroy() noexcept { LOG_INFO("Total events processed: {}", event_count_); }
...
bool ExampleMIDIPrinterPlugin::activate(const applause::ProcessInfo& info) noexcept {
    ...
    if (host_dialects & CLAP_NOTE_DIALECT_CLAP) LOG_INFO("  - CLAP (native)");
    if (host_dialects & CLAP_NOTE_DIALECT_MIDI) LOG_INFO("  - MIDI 1.0");
    if (host_dialects & CLAP_NOTE_DIALECT_MIDI_MPE) LOG_INFO("  - MIDI MPE");
    if (host_dialects & CLAP_NOTE_DIALECT_MIDI2) LOG_INFO("  - MIDI 2.0");

    return true;
}

void ExampleMIDIPrinterPlugin::deactivate() noexcept { LOG_INFO("Events processed in this session: {}", event_count_); }
...
applause::ProcessStatus ExampleMIDIPrinterPlugin::process(applause::ProcessContext& context) noexcept {
    ...
    if (!context.audioOutputs().empty()) context.output<float>().clear();
...
void ExampleMIDIPrinterPlugin::printNoteEvent(const clap_event_note_t* event, const char* event_name) {
    ...
    auto key_str =
        event->key >= 0 ? std::to_string(event->key) + " (" + getNoteNameFromKey(event->key) + ")" : "wildcard";
    auto note_id_str = event->note_id >= 0 ? std::to_string(event->note_id) : "unspecified";
    LOG_INFO("  {}: port={} ch={} key={} vel={:.3f} id={}", event_name, port_str, chan_str, key_str, event->velocity,
             note_id_str);
}
...
void ExampleMIDIPrinterPlugin::printNoteExpression(const clap_event_note_expression_t* event) {
    ...
    case CLAP_NOTE_EXPRESSION_PAN:
        interpretation = std::format(" [{:.0f}% {}]", std::abs(event->value - 0.5) * 200,
    ...
    LOG_INFO("  NOTE_EXPRESSION: {} port={} ch={} key={} id={} val={:.6f}{}", getExpressionName(event->expression_id),
             port_str, chan_str, key_str, note_id_str, event->value, interpretation);
}
...
void ExampleMIDIPrinterPlugin::printMidiEvent(const clap_event_midi_t* event) {
    ...
    case 0x90:  // Note On
        ...
        if (status == 0x90 && event->data[2] == 0) details += " [vel=0 treated as Note Off]";
        break;
    case 0xA0:  // Polyphonic Aftertouch
        details =
            std::format("key={} ({}) pressure={}", event->data[1], getNoteNameFromKey(event->data[1]), event->data[2]);
        break;
    ...
    LOG_INFO("  MIDI: [{:02X} {:02X} {:02X}] port={} ch={} {} {}", event->data[0], event->data[1], event->data[2],
...
void ExampleMIDIPrinterPlugin::printMidi2Event(const clap_event_midi2_t* event) {
    ...
    switch (message_type) {
    case 0x2:
        msg_type_str = "MIDI1.0 Chan Voice";
        break;
    case 0x3:
        msg_type_str = "64-bit Data";
        break;
    case 0x4:
        {
        ...
        break;
    case 0x5:
        msg_type_str = "128-bit Data";
        break;
    default:
        msg_type_str = std::format("type={:X}", message_type);
        break;
    }

    LOG_INFO("  MIDI2: [{:08X} {:08X} {:08X} {:08X}] port={} group={} {}", event->data[0], event->data[1],
             event->data[2], event->data[3], event->port_index, group, msg_type_str);
}
...
void ExampleMIDIPrinterPlugin::printMidiSysexEvent(const clap_event_midi_sysex_t* event) {
    ...
    if (event->buffer && event->size > 0) {
        ...
        for (size_t i = 0; i < bytes_to_print; ++i) {
            ss << std::hex << std::setw(2) << std::setfill('0') << static_cast<int>(event->buffer[i]) << " ";
        }
        if (event->size > 16) ss << "... (+" << (event->size - 16) << " more)";
        data_str = ss.str();
    }
